...
<script>
    // 生成中奖号码的方法
    let getLotteryNum= function(){
        return Math.trunc(Math.random()*10+1)
    }
    // 定义一个promise对象
    let promise =new Promise(function(resolve,reject){
        console.log("已经买了彩票,等待开奖")
        setTimeout(function (){
            let num =getLotteryNum();        
            if(num>= 5){
                resolve(`中奖了,号码为:${num}`)
            }else{
                reject(`未中奖,号码为:${num}`)
    ...
    promise.then(
        function(message){
            console.log(`两天后:${message}`)
        },
        function(error){
            console.log(`两天后:${error}`)
        }
    )
    console.log('other code2222 invoked')
</script>
...
<script>
    ...
    // 调用函数
    let res =fun1()
    // 函数返回的是一个promise对象,调用then方法设置不同状态的回调函数
    res.then(
        (value)=>{
            console.log(value)
        },
        ...
        <table class="table table-hover table-striped table-bordered">
           ...
           <tbody v-if="carts.length > 0">
               ...
               <tr v-for="cart,index in carts" :key="index">
                  ...
                  <th> <button class="btn btn-denger" @click="removeCart(index)">删除</button> </th>
               </tr>
           </tbody>
           <tbody v-else>
               <!-- 没有数据显示-->
...
![](images/image_UDJErYr8iM.png)

+ 处理方案：导入的scss格式文件，需要导入依赖解析！
...
<template>
    <div>
      ...
      <hr/>
        ...
        <router-link to="/add">add页</router-link> <br>
        <router-link to="/update">update页</router-link> <br>
        <!-- 动态输入路径,点击按钮,触发单击事件昂丁的函数,在函数中通过编程是路由切换页面 -->
...
<script setup type="module">
  ...
  let getLoveMessage =()=>{
    getLoveWords().then( function (response){
      console.log(response)
      Object.assign(jsonData,response.data)
...
<img src="./images/image_IJOzvAeodc.png" style="zoom:50%;" />
...
+ 开发组件
    +  List.vue      信息展示页/list
    +  Error.vue   错误页面 /error   (创建放显示文本即可)
    +  Error404.vue 资源找不到页 /404  (创建放显示文本即可)
...
+ src/axios/axios.js      axios需要将对象配置到vue对象中，可以配置前后置拦截器，统一处理错误状态码等 ！
...
+ 我们本次使用pinia存储json数据和对应的操作方法！创建pinia存储文件store/store.js
...
//定义数据并且对外暴漏
//state存储一个JSON数据
export const scheduleStore = defineStore(
    {
        id: 'data', //必须唯一
        state:()=>{
            return {
...
+ 将数据存储到一个本地JSON文件 /src/assets/data.json
...
-   进入List.vue（生命周期）,请求第一页数据
-   调用store.js对应方法，进行数据加载
-   获取store.js返回的数据，进行页面状态展示即可

> 1 定义store.js state 和 action 和 getter
...
export const scheduleStore = defineStore(
    {
        ...
        getters:{
            ...
            currentPageData:(state)=>{
                // 根据页码数和页大小截取数组中本页的数据
                return state.jsondata.list.slice((state.currentPage-1) * state.pageSize,state.currentPage*state.pageSize)
            }
            
        },
        actions:{
           // 通过axios发送异步请求,读取data.json文件获取数据 
           // page 给定默认值为1
           // 通过axios发送异步请求,读取data.json文件获取数据 
           // page 给定默认值为1
           async getList(page = 1){
                ...
                if(!this.loaded){
                    // 结构响应数据中的data
                    const {data} = await axios.get('/src/assets/data.json') 
                    // 将获取的数据放在state中定义的属性里
                    this.jsondata=data
                    
                    // 加载完毕后,修改是否加载标记
                    this.loaded=true
                    // 根据元素数量,修改最大id
                    this.maxId=data.list.length
                }
                // 修改分页数据
                // 设置当前页码
                this.currentPage=page
                // 获取所有数据总长度
                this.total = this.jsondata.list.length
           }
        }
    }
)
...
> 3 List.vue引入store.js

```html
<script setup type="module">

import { onMounted } from 'vue'
import { scheduleStore } from '../store/store.js'

// 获取数据对象
const schedule = scheduleStore();
// 挂载完毕后,读取数据,自动获取最新全部数据存于pinia中
onMounted(() => {
   schedule.getList();
})
//自定义标签 编辑标签触发方法
...
//点击分页触发的方法
const handlePageChange = page => schedule.getList(page)
...
<template>
   <div class="common-layout">
      <el-container>
         <el-main>
            <!-- 表格 -->
            <el-table :data="schedule.currentPageData" style="width: 100%">
...
-   点击删除按钮，出发删除方法，传入要删除的id 
-   store.js中 actions 定义删除方法，进行state数据更新
-   注意：本次不做本地数据更新，后期访问后台即可

> 1 store.js中,actions添加代码

```javascript
// 根据数据 的id属性删除数据的方法
async removeItem(id){
    // 根据id获取元素在数组中的索引
    const index= this.jsondata.list.findIndex((item)=> item.id == id)
    // 根据索引删除元素
    this.jsondata.list.splice(index,1)
    // 总数量-1
    this.total -= 1
    // 处理总页数 如果总页数小于当前页,就设置为最后一页
...
1.  点击修改按钮，进行数据回显（dialog）
2.  确认修改，调用store.js方法替换数据即可
...
async updateItem(newItem){
    //1.查找ID在数据中的下角标
    const index = this.jsondata.list.findIndex( item => item.id== newItem.id)
    //2.替换数据state数据,修改state总条数
    this.jsondata.list.splice(index,1,newItem)
}
```
   ...
   <div class="common-layout">
      <el-container>
         <el-main>
            ...
            <br>
            <div class="pagination">
               <el-pagination background layout="prev, pager, next" :total="schedule.total"
                  :current-page="schedule.currentPage" :page-size="schedule.pageSize" @current-change="handlePageChange" />
            </div>
...
1.  List.vue添加一个 添加按钮
2.  点击添加按钮，跳转到添加Add.vue
3.  内部收集数据，并且调用store.js addItem方法保存
4.  最终返回的到展示页面即可

> 1 store.js的 actions中增加addItem方法
...
async addItem(newItem){
    ...
    //  将新元素放入数据
    this.jsondata.list.push(newItem)
    //  更新总记录数
    this.total += 1
}
```

>  2 components/AddItem.vue
...
![1684827427264](images/1684827427264.png)